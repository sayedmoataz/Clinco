...
class AdminRaysList extends StatefulWidget {
  final AccountStatus accountStatus;

  const AdminRaysList({Key? key, required this.accountStatus})
      : super(key: key);

  @override
  _AdminRaysListState createState() => _AdminRaysListState();
}

class _AdminRaysListState extends State<AdminRaysList> {
  Color primaryLightColor = AppColors.primaryColor;
  List<Color> primaryGradientColors = AppColors.primaryGradientColors;
  BuildContext? dialogContext;
  List<Rays> rays = <Rays>[];
  FirebaseFirestore? _firebaseFirestore;
  late CollectionReference raysRef, usersRef;
  ...
  void initState() {
    ...
    raysRef =
        _firebaseFirestore!.collection(FirestoreCollections.RaysCenter.name);
              ...
              builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                if (snapshot.hasData) {
                  ...
                  map.forEach((dynamic, json) {
                    rays.add(Rays.fromJson(json.data()));
                  });
                                                    ...
                                                    child: Text(
                                                        ...
                                                        style: const TextStyle(
                                                            ...
                                                            color:
                                                                Colors.white))),
                                                // const Spacer(flex: 1,),
                                                // Flexible(flex: 3, fit:  FlexFit.loose, child: Text(rays[i].doctorLevel ?? '', overflow: TextOverflow.ellipsis, maxLines: 1, textDirection: TextDirection.rtl, style: const TextStyle(fontSize: 10.0, fontWeight: FontWeight.bold, color: AppColors.secondaryColor2))),
                                                const Spacer(
                                                  flex: 1,
                                                ),
                                                Flexible(
