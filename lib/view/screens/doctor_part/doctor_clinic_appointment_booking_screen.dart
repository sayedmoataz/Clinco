...
class DoctorClinicAppointmentBookingScreen extends StatefulWidget {
  final Clinic? clinic;
  final String? clinicId;
  final Doctor? doctor;
  final Speciality? clinicSpeciality;

  const DoctorClinicAppointmentBookingScreen(
      {Key? key,
  ...
  @override
  _DoctorClinicAppointmentBookingScreenState createState() =>
      _DoctorClinicAppointmentBookingScreenState();
}

class _DoctorClinicAppointmentBookingScreenState
    extends State<DoctorClinicAppointmentBookingScreen> {
  ...
  String? _patientDoctorId, _patientDoctorUserId;
  ...
  void initState() {
    super.initState();
    clinicBookingRequestsRef = _firebaseFirestore
        .collection(FirestoreCollections.ClinicBookingRequests.name);
    includesOnlinePaymentOffer =
    ...
    _appData.getSharedPreferencesInstance().then((pref) {
      _patientDoctorUserId = _appData.getUserId(pref!)!;
      _getPatientData();
    });
  }
  ...
  void _getPatientData() {
    _firebaseFirestore
        ...
        .where('userId', isEqualTo: _patientDoctorUserId)
        .get()
        .then((snapshots) {
      ...
      if (queryDocumentSnapshot.exists) {
        setState(() {
          _patientDoctorId = queryDocumentSnapshot.reference.id;
          _patientDoctor = Doctor.fromJson(queryDocumentSnapshot.data());
        });
      }
    });
  }
    ...
    return Card(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          // Expanded(flex: 2, child: _getManageIconsColumn(docs[i].reference.path, dialog)),
        ]),
      ),
    );
  }
 ...
 _buildPaymentMethodCardWidget(String title, String subtitle, IconData icon, VoidCallback callback) {
  ...
  _buildPaymentWaysSelectionWidget() {
    return HomeCubit.get(context).isLoadingState ? const CircularProgressIndicator()
    : Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      mainAxisAlignment: MainAxisAlignment.start,
      children: [
        _buildPaymentMethodCardWidget(
          ...
          (){
            ...
            //debugPrint("_patient!.name!.toString() is : ${_patientDoctor!.name.toString()}\n _patient!.name! is : ${_patientDoctor!.name!.toString()}\n_patient!.phoneNumber! is : ${_patientDoctor!.phoneNumber!.toString()}\n_patient!.email! is : ${_patientDoctor!.email!.toString()}\nwidget.clinic!.price is : ${price}\nIntegrationIDKiosk is : ${IntegrationIDKiosk.toString()}");
            HomeCubit.get(context).getAuthToken(
              _patientDoctor!.name!.toString(),
              _patientDoctor!.name!.toString(),
              _patientDoctor!.phoneNumber!.toString(),
              _patientDoctor!.email!.toString(),
        ...
        _buildPaymentMethodCardWidget(
          ...
          FontAwesomeIcons.ccVisa,
             (){
                price = widget.clinic!.price - widget.clinic!.revealWay;
                //debugPrint("_patient!.name!.toString() is : ${patient!.name!.toString()}\n _patient!.name! is : ${patient!.name!.toString()}\n_patient!.phoneNumber! is : ${patient!.phoneNumber!.toString()}\n_patient!.email! is : ${patient!.email!.toString()}\nwidget.clinic!.price is : ${widget.clinic!.price.toString()}\nIntegrationIDKiosk is : ${IntegrationIDKiosk.toString()}");
                HomeCubit.get(context).getAuthToken(
                  _patientDoctor!.name!.toString(),
                  _patientDoctor!.name!.toString(),
                  _patientDoctor!.phoneNumber!.toString(),
  ...
  _buildAvailableTimeWidget(
      ClinicAvailableTimeSlot clinicAvailableTimeSlot, bool isBookedTime) {
    return Card(
  ...
  requestBooking(context, LoadingIndicator dialog) async {
    BookingRequest bookingRequest = BookingRequest(
        ...
        _patientDoctorUserId,
        _patientDoctorId,
        widget.clinicId,
        _patientDoctor?.name,
        _patientDoctor?.phoneNumber,
        _patientDoctor?.email,
        _symptoms,
